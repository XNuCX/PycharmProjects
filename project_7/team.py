...
class Team:
    ...
    def add_player(self, player:Player):
        # player_for_add = [p for p in self.__players if not p.name == player.name]
        if player.name not in [s.name for s in self.__players]:
    ...
    def remove_player(self, player_name:str):
        index = [i for i, o in enumerate(self.__players) if o.name == player_name]
        try:
            return self.__players.pop(index[0])
        except IndexError:
