from project_20.library import Library
...
class Registration:
    ...
    def add_user(self, user: User, library: Library):
        if user.user_id not in [user_temp.user_id for user_temp in library.user_records]:
    ...
    def remove_user(self, user: User, library: Library):
        if user.user_id in [user_temp.user_id for user_temp in library.user_records]:
            library.user_records = [user_temp for user_temp in library.user_records if not user_temp.user_id == user.user_id]
            return
        else:
            return f"We could not find such user to remove!"

    def change_username(self, user_id: int, new_username: str, library: Library):
        if user_id in [u.user_id for u in library.user_records]:
            if new_username not in [u.username for u in library.user_records]:
                for _ in library.user_records:
                    if _.user_id == user_id:
                        if _.username in library.rented_books:
                            library.rented_books[new_username] = library.rented_books.pop(_.username)
